/******************************************************************
Change Script 2.0.8.44 - consolidated.
1. pmt_project - updating to remove sumation of financial information
and replacing with a financial object.
******************************************************************/
INSERT INTO version(version, iteration, changeset) VALUES (2.0, 8, 44);
-- select * from version order by changeset desc;

-- select * from pmt_project(731);
...
BEGIN
  IF $1 IS NOT NULL THEN	
    ...
    execute_statement := execute_statement || ',(SELECT array_to_json(array_agg(row_to_json(p))) FROM ( ' ||
				...
				'left join taxonomy_classifications tc ' ||
    ...
    execute_statement := execute_statement || ',(SELECT array_to_json(array_agg(row_to_json(f))) FROM ( ' ||
				'select f.financial_id, f.amount, tc.taxonomy_id, tc.taxonomy, tc.classification_id, tc.classification ' ||
				'from financial f ' ||	
