/******************************************************************
Change Script 2.0.8.31
1. bmgf_filter_csv - add region to export
******************************************************************/
INSERT INTO version(version, iteration, changeset) VALUES (2.0, 8, 31);
-- select * from version order by changeset desc;

-- select * from bmgf_filter_csv('244','','',null,null, 'sparadee@spatialdev.com');
...
BEGIN
  ...
  FOREACH pid IN ARRAY project_ids LOOP
	...
	INSERT INTO csv_data (id,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18)	 
		...
		(select pp.project_id, array_to_string(array_agg(DISTINCT o.name), ',') as name, array_to_string(array_agg(o.url), ',') as url
		...
		left join
		-- focus crop
		(select p.project_id, array_to_string(array_agg(DISTINCT c.name), ',') as focus_crop
		from project p 
		...
		(select pp.project_id, array_to_string(array_agg(DISTINCT o.name), ',') as sub_grantees
		from participation pp
		join organization o
		on pp.organization_id = o.organization_id
		left join participation_taxonomy ppt
		...
		left join
		-- country
		(select pt.project_id, array_to_string(array_agg(DISTINCT c.name), ',') as country 
		from project_taxonomy pt
		join classification c
		on pt.classification_id = c.classification_id
     ...
     INSERT INTO csv_data (id,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17)        	
     		SELECT  counter
			,'Activity Data' 
			,'PMT ActivityID'
			,'OppID'
	...
	INSERT INTO csv_data (id,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17)         	
		...
		left join
		-- all partners
		(select pp.activity_id, array_to_string(array_agg(DISTINCT o.name), ',') as partners, array_to_string(array_agg(DISTINCT c.name), ',') as role
		from participation pp
		join organization o
		on pp.organization_id = o.organization_id
		left join participation_taxonomy ppt
		...
		group by ll.activity_id) g
		on a.activity_id = g.activity_id;
        counter := counter + 1;
     END LOOP;     
  END LOOP;
