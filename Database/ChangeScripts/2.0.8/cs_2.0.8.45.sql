/******************************************************************
Change Script 2.0.8.45 - consolidated.
1. pmt_activity - updating to remove sumation of financial information
and replacing with a financial object.
2. pmt_project - adding nested taxonomy objects for objeccts reporting
taxonomy instead of duplicating records.
******************************************************************/
INSERT INTO version(version, iteration, changeset) VALUES (2.0, 8, 45);
...
BEGIN
  IF $1 IS NOT NULL THEN	
    ...
    execute_statement := execute_statement || ',(SELECT array_to_json(array_agg(row_to_json(p))) FROM ( ' ||
				'select o.organization_id, o.name, o.url'  ||
