/******************************************************************
Change Script 2.0.8.65
1. pmt_activity - add dates to financial object returned.
2. pmt_project - add dates to financial object returned.
******************************************************************/
INSERT INTO version(version, iteration, changeset) VALUES (2.0, 8, 65);
-- select * from version order by changeset desc;

/******************************************************************
 TESTING
	
 select * from pmt_activity(11850);  
 select * from pmt_activity(2); --null
 select * from pmt_activity(10815);    

 select * from activity where active = true;

 select * from pmt_project(732);    
 select * from pmt_project(2);    
 select * from pmt_project(9999);    
...
BEGIN
  IF $1 IS NOT NULL THEN	
    ...
    execute_statement := execute_statement || ',(SELECT array_to_json(array_agg(row_to_json(f))) FROM ( ' ||
				'select f.financial_id, f.amount, f.start_date, f.end_date '  ||
						', (SELECT array_to_json(array_agg(row_to_json(t))) FROM ( ' ||
						'select tc.taxonomy_id, tc.taxonomy, tc.classification_id, tc.classification, tc.code ' ||
