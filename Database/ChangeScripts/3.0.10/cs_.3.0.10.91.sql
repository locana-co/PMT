/******************************************************************
Change Script 3.0.10.91
1. update pmt_activity function to add organization type
******************************************************************/
INSERT INTO version(_version, _iteration, _changeset) VALUES (3.0, 10, 91);
...
BEGIN
  IF $1 IS NOT NULL THEN	
    ...
    execute_statement := execute_statement || ',(SELECT array_to_json(array_agg(row_to_json(p))) FROM ( ' ||
				...
				'_city as city, _state_providence as state_providence, _postal_code as zip, _country as country, role, array_to_string(array_agg(type),'','') as type ' ||
				'from ( ' ||
