/******************************************************************
Change Script 3.0.10.49
1. update function pmt_boundary_pivot to fix errors 
******************************************************************/
INSERT INTO version(_version, _iteration, _changeset) VALUES (3.0, 10, 49);
...
1. update function pmt_boundary_pivot to fix errors 
   ...
   select * from pmt_boundary_pivot(19,69,false,497,'2237',null,null,null,15,74);
   select * from pmt_boundary_pivot(19,68,true,497,'2237',null,null,null,12,3);
******************************************************************/
...
BEGIN
  ...
  IF(boundary_as_row)THEN
    SELECT INTO column_headers array_agg(classification)::text[] FROM _taxonomy_classifications WHERE taxonomy_id = valid_pivot_taxonomy_id;
    EXECUTE 'SELECT array_agg(id)::int[] FROM '|| pivot_table ||' WHERE _'|| spatial_table || '_name = ' || quote_literal(feature_name) INTO feature_ids;
  ELSE      
    EXECUTE 'SELECT array_agg(DISTINCT _name)::text[] FROM '|| pivot_table ||' WHERE _'|| spatial_table || '_name = ' || quote_literal(feature_name) INTO column_headers;
  END IF;  
  ...
  FOREACH col IN ARRAY column_headers LOOP
    execute_statement:= execute_statement || 'to_json(' || quote_literal(col) || '::text) as c' || col_count || ','; 
  ...
  FOREACH col IN ARRAY column_headers LOOP
    execute_statement:= execute_statement || 'json_agg(distinct case when cols.classification = ' || quote_literal(col) || 
