/******************************************************************
Change Script 3.0.10.108
1. update pmt_activities_all 
******************************************************************/
INSERT INTO version(_version, _iteration, _changeset) VALUES (3.0, 10, 108);
-- select * from version order by _iteration desc, _changeset desc;

/******************************************************************
4. update pmt_activities_all
   select * from pmt_activities_all('2237', false); -- all activities active true & false
   select * from pmt_activities_all(null, false);
******************************************************************/
...
BEGIN
  ...
  execute_statement:= 'SELECT DISTINCT a.id, parent_id as pid, data_group_id as dgid, _iati_identifier as iati, (SELECT _name FROM classification WHERE id = data_group_id) as dg, ' ||
		'_title as t, _amount as a, a._start_date as sd, a._end_date as ed, a._active x, array_agg( o._name) as f, array_agg(DISTINCT v.classification) as v ' ||
		'FROM ( SELECT DISTINCT af.parent_id as id, null as parent_id, _iati_identifier, af.data_group_id, af._title, a._start_date, a._end_date, a._active ' ||
			'FROM ( ' || 
  ...
  execute_statement:= execute_statement || ') af JOIN activity a ON af.parent_id = a.id ) a' ||
		...
		'LEFT JOIN (SELECT id, classification FROM _activity_taxonomies WHERE taxonomy = ''Version'') v ON a.id = v.id ' ||
...
SELECT DISTINCT a.id, parent_id as pid, data_group_id as dgid, _iati_identifier as iati, 
	(SELECT _name FROM classification WHERE id = data_group_id) as dg, _title as t, _amount as a, 
	a._start_date as sd, a._end_date as ed, a._active as x, array_agg(DISTINCT o._name) as f, array_agg(DISTINCT v.classification) as v 
FROM ( 
	SELECT DISTINCT af.parent_id as id, null as parent_id, _iati_identifier, af.data_group_id, af._title, a._start_date, a._end_date, a._active
	FROM ( 
		SELECT p.id AS parent_id, c.id AS child_id, p._title, p.data_group_id 
		FROM ( 
			SELECT activity.id, activity._title, activity.data_group_id 
			FROM activity 
			WHERE activity.parent_id IS NULL AND activity.data_group_id = ANY(ARRAY[2237])
		) p 
		LEFT JOIN ( 
			SELECT activity.id, activity.parent_id FROM activity 
			WHERE activity.parent_id IS NOT NULL AND activity.data_group_id = ANY(ARRAY[2237]) 
		) c 
		ON p.id = c.parent_id 
	) af 
	JOIN activity a 
	ON af.parent_id = a.id 
) a 
LEFT JOIN ( 
	SELECT DISTINCT f.activity_id, sum(_amount) as _amount 
	FROM ( 
		SELECT id, activity_id, _amount, provider_id 
		FROM financial 
		WHERE _active = true
	) f  
	LEFT JOIN ( 
		select financial_id, classification, _code  
		FROM financial_taxonomy ft  
		JOIN _taxonomy_classifications tc  
		on ft.classification_id = tc.classification_id  
		where tc.taxonomy = 'Transaction Type'OR classification IS NULL OR classification = 'Incoming Funds' OR classification = 'Commitment' 
	) as ft 
