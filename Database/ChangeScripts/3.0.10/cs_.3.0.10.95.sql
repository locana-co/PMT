/******************************************************************
Change Script 3.0.10.95
1. detail_taxonomy table version 3.0 changes
2. update pmt_activity function to add implementing type
******************************************************************/
INSERT INTO version(_version, _iteration, _changeset) VALUES (3.0, 10, 95);
-- select * from version order by _iteration desc, _changeset desc;

/******************************************************************
1. detail_taxonomy table version 3.0 changes
******************************************************************/
ALTER TABLE detail_taxonomy RENAME COLUMN field TO _field;

/******************************************************************
2. update pmt_activity function to add implementing type
  SELECT * FROM pmt_activity(29692); 
******************************************************************/
...
BEGIN
  IF $1 IS NOT NULL THEN	
    ...
    execute_statement := execute_statement || ',(SELECT array_to_json(array_agg(row_to_json(p))) FROM ( ' ||
				...
				'_city as city, _state_providence as state_providence, _postal_code as zip, _country as country, role, array_to_string(array_agg(t.type),'','') as type, i.type as prime ' ||
				'from ( ' ||
				'select organization_id, _name, _url, _address1, _city, _state_providence, _postal_code, _country, classification as role ' ||
				'from (select * from _organization_lookup where taxonomy = ''Organisation Role'') orgrole  ' ||
				'join organization o  ' ||
				'on orgrole.organization_id = o.id ' ||
				'where activity_id = ' ||  $1 || ') r ' ||
				'left join  ' ||
				'(select organization_id, classification as type ' ||
				'from ' ||
				'(select * from _organization_lookup where taxonomy = ''Organisation Type'') orgtype ' || 
				'join organization o ' || 
				'on orgtype.organization_id = o.id ' ||
				'where activity_id = ' ||  $1 || ') as t ' ||
				'on r.organization_id = t.organization_id ' ||
				'left join ' ||  
				'(select organization_id, classification as type ' || 
				'from ' || 
				'(select * from _organization_lookup where taxonomy = ''Implementing Types'') orgtype ' ||  
				'join organization o ' ||  
				'on orgtype.organization_id = o.id ' || 
				'where activity_id = ' ||  $1 || ') as i ' || 
