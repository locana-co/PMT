/******************************************************************
Change Script 3.0.10.41
1. update pmt_upd_boundary_features to allow custom points for 
polygon features
******************************************************************/
INSERT INTO version(_version, _iteration, _changeset) VALUES (3.0, 10, 41);
...
BEGIN
  ...
  -- if a boundary_id and feature_id are provided (without a point) then get a centroid of the requested feature to serve as the point
  -- locations can be polygons by making an association to an existing boundary feature
  IF ((SELECT * FROM pmt_validate_boundary_feature(NEW.boundary_id, NEW.feature_id)) AND NEW._point IS NULL) THEN
