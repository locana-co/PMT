/******************************************************************
Change Script 3.0.10.39
1. create new function pmt_boundary_pivot
******************************************************************/
INSERT INTO version(_version, _iteration, _changeset) VALUES (3.0, 10, 39);
-- select * from version order by _iteration desc, _changeset desc;

/******************************************************************
1. create pmt_boundary_pivot
   select * from pmt_boundary_pivot(16,69,false,496,'2237',null,null,null,15,74);
...
BEGIN
  ...
  IF (boundary_as_row) THEN
    execute_statement:= execute_statement	|| '(SELECT id, classification ' ||
			...
			'WHERE activity_id = ANY(ARRAY[' || array_to_string(filtered_activity_ids, ',') || ']) AND boundary_id = ' || valid_pivot_boundary_id || ') as rows ' ||
			'ON a.id = rows.id ' ||
			'GROUP BY 1 ' ||
			') as selection';
  ELSE
